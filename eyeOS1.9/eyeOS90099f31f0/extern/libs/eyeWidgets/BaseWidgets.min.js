...
function eyeTab(b,c,d,e){this.csignal=c;this.checknum=d;this.counter=0;this.lastClick=null;this.tabWidth=e;this.initialOffset=0;this.tabHeight=23;this.myName=b.id;this.moTab=b;this.myTabs=[];this.ontabselect=false;var f=this;c=document.createElement("div");c.setAttribute("id",b.id+"_header");c.className="eyeTabHeader";d=document.createElement("div");e=document.createElement("div");d.className="eyeTabArrowLeft";e.className="eyeTabArrowRight";d.setAttribute("id",b.id+"_buttonLeft");d.onmousedown=function(){this.interval=
setInterval('document.getElementById("'+b.id+'_header").scrollLeft-=10',50)};d.onmouseup=function(){clearTimeout(this.interval)};d.onmouseout=function(){clearTimeout(this.interval)};e.setAttribute("id",b.id+"_buttonRight");e.onmousedown=function(){this.interval=setInterval('document.getElementById("'+b.id+'_header").scrollLeft+=10',50)};e.onmouseup=function(){clearTimeout(this.interval)};e.onmouseout=function(){clearTimeout(this.interval)};c.style.width=String(xWidth(b)-xWidth(d)-xWidth(e))+"px";
d.style.display="none";e.style.display="none";b.appendChild(d);b.appendChild(c);b.appendChild(e);this.oHeader=c;this.oHeader.onclick=function(g){f.headerClick(g)};this._onunload=function(){f.destroy()}}a=eyeTab.prototype;a.getCurrentTab=function(){return this.lastClick};a.headerClick=function(b){b=new xEvent(b);if(b.target.className=="eyeTabText")b.target=b.target.parentNode;if(b.target.className=="eyeTabSelected"||b.target.className=="eyeTabNotSelected")this.selectTab(b.target.id)};
a.addTab=function(b,c,d){var e=this,f=document.createElement("div"),g=document.createElement("div");g.id=this.myName+"_"+c+"_Title";g.innerHTML=b;g.className="eyeTabText";f.className="eyeTabNotSelected";f.setAttribute("id",this.myName+"_"+c);f.style.width=this.tabWidth+"px";f.style.position="absolute";f.style.top="0px";f.style.height=this.tabHeight+"px";c=1;if(IEversion&&IEversion<7)c=0;this.counter++;c=(this.counter-1)*(Number(this.tabWidth)+c)+this.initialOffset;f.style.left=c+"px";f.appendChild(g);
this.oHeader.appendChild(f);g=document.createElement("div");g.setAttribute("id",f.id+"_Content");g.className="eyeTabBody";g.style.width=xWidth(this.moTab)+"px";var l=xHeight(this.moTab)-xHeight(this.oHeader);g.style.height=l+"px";this.moTab.appendChild(g);this.selectTab(f.id);if(!d){d=document.createElement("div");d.setAttribute("id",b.id+"_cross");d.className="eyeTabCross";d.onclick=function(){sendMsg(e.checknum,e.csignal,eyeParam("arg",f.id))};f.appendChild(d)}this.myTabs[f.id]=c;if(this.oHeader.childNodes.length*
xWidth(f)>xWidth(this.oHeader)){document.getElementById(this.moTab.id+"_buttonLeft").style.display="block";document.getElementById(this.moTab.id+"_buttonRight").style.display="block"}else{document.getElementById(this.moTab.id+"_buttonLeft").style.display="none";document.getElementById(this.moTab.id+"_buttonRight").style.display="none"}};
a.removeTab=function(b){var c=document.getElementById(b);if(c){if(this.oHeader.childNodes.length-1*xWidth(c)>xWidth(this.oHeader)){document.getElementById(this.moTab.id+"_buttonLeft").style.display="block";document.getElementById(this.moTab.id+"_buttonRight").style.display="block"}else{document.getElementById(this.moTab.id+"_buttonLeft").style.display="none";document.getElementById(this.moTab.id+"_buttonRight").style.display="none"}var d=false,e=true;if(b!==this.lastClick)e=false;for(var f in this.myTabs)if(this.myTabs[f]!==
null){if(f===b)d=true;else if(e!==false&&d===false)e=f;else if(e===true)e=f;if(this.myTabs[f]>this.myTabs[b]){this.myTabs[f]-=this.tabWidth;document.getElementById(f).style.left=this.myTabs[f]+"px"}}d=document.getElementById(c.id+"_Content");this.oHeader.removeChild(c);this.moTab.removeChild(d);this.counter--;this.myTabs[b]=null;e!==false&&e!==true&&this.selectTab(e);this.oHeader.scrollLeft=this.oHeader.scrollLeft}};
a.selectTab=function(b){var c=document.getElementById(b);if(c)if(c.id!=this.lastClick){c.className="eyeTabSelected";c=document.getElementById(c.id+"_Content");if(this.moTab.useDisplay)c.style.display="block";else c.style.visibility="visible";if(this.lastClick)if(c=document.getElementById(this.lastClick)){c.className="eyeTabNotSelected";c=document.getElementById(c.id+"_Content");if(this.moTab.useDisplay)c.style.display="none";else c.style.visibility="hidden"}this.lastClick=b;typeof this.ontabselect===
"function"&&this.ontabselect(this.lastClick)}};
function Textarea_show(b,c,d,e,f,g,l,k,m){var h=b.width,j=b.height,p=b.rich,n=b.code,o=b.lang,t=b.rows,q=b.cols,u=b.enabled,v=b.cssClass,A=b.visible;b=b.language;var s=document.createElement("textarea");if(n==1){s.id=c+"_cp";s.name=c+"_cp"}else{s.setAttribute("id",c);s.setAttribute("name",c)}t&&s.setAttribute("rows",t);q&&s.setAttribute("cols",q);if(h>0)s.style.width=h+"px";if(j>0)s.style.height=j+"px";if(!u)s.disabled=1;if(v){s.className=v;createWidget(c+"_Container",d,s,g,l,e,f,h,j,v,m,"px",A,"Textarea")}else{s.className=
...
Hide:function(b){var c=document.getElementById(b);if(c){if(Windows.List[b].showDragContent)c.style.visibility="hidden";else c.style.display="none";if(IEversion&&IEversion<7){c=c.getElementsByTagName("select");for(var d=0;d<c.length;d++)if(c[d].style.visibility!="hidden"){c[d].name="visible";c[d].style.visibility="hidden"}}typeof Windows.List[b].JS==="object"&&typeof Windows.List[b].JS.Hide==="function"&&Windows.List[b].JS.Hide(b)}},MaxHeight:function(b){var c=document.getElementById(b);if(c&&!Windows.List[b].noResize&&
Windows.List[b].maximized!=2){var d;if(Windows.List[b].maximized){c=Windows.List[b].height;Windows.SetHeight(b,c,1);d=Windows.List[b].width;Windows.SetWidth(b,d,1);Windows.SetX(b,Windows.List[b].x);Windows.SetY(b,Windows.List[b].y);Windows.List[b].maximized=0}else{if(!Windows.List[b].maximized){Windows.List[b].height=xHeight(b);Windows.List[b].width=xWidth(b);Windows.List[b].x=xLeft(b);Windows.List[b].y=xTop(b)}c=xHeight(c.parentNode);Windows.SetHeight(b,c,2);d=Windows.List[b].width;Windows.SetY(b,
0,1);Windows.List[b].maximized=1}typeof Windows.List[b].resizeMessage!="undefined"&&sendMsg(Windows.List[b].checknum,Windows.List[b].resizeMessage,eyeParam("arg",d)+eyeParam("arg",c))}},Maximize:function(b){var c=document.getElementById(b);if(c&&!Windows.List[b].noResize){var d;if(Windows.List[b].maximized==2){d=Windows.List[b].height;Windows.SetHeight(b,d,1);c=Windows.List[b].width;Windows.SetWidth(b,c,1);Windows.SetX(b,Windows.List[b].x);Windows.SetY(b,Windows.List[b].y);Windows.List[b].maximized=
0}else{if(!Windows.List[b].maximized){Windows.List[b].height=xHeight(b);Windows.List[b].width=xWidth(b);Windows.List[b].x=xLeft(b);Windows.List[b].y=xTop(b)}d=xHeight(c.parentNode);Windows.SetHeight(b,d,2);c=xWidth(c.parentNode);Windows.SetWidth(b,c,2);Windows.SetX(b,0,1);Windows.SetY(b,0,1);Windows.List[b].maximized=2}typeof Windows.List[b].resizeMessage!="undefined"&&sendMsg(Windows.List[b].checknum,Windows.List[b].resizeMessage,eyeParam("arg",c)+eyeParam("arg",d))}},MaximizeButton:function(b,c){if(b=
document.getElementById(b+"_WindowMaxButton"))b.style.display=c==2&&b.style.display!="none"||typeof c!="undefined"&&!c?"none":"block"},MaxWidth:function(b){var c=document.getElementById(b);if(c&&!Windows.List[b].noResize&&Windows.List[b].maximized!=2){var d;if(Windows.List[b].maximized){d=Windows.List[b].height;Windows.SetHeight(b,d,1);c=Windows.List[b].width;Windows.SetWidth(b,c,1);Windows.SetX(b,Windows.List[b].x);Windows.SetY(b,Windows.List[b].y);Windows.List[b].maximized=0}else{if(!Windows.List[b].maximized){Windows.List[b].height=
xHeight(b);Windows.List[b].width=xWidth(b);Windows.List[b].x=xLeft(b);Windows.List[b].y=xTop(b)}d=Windows.List[b].height;c=xWidth(c.parentNode);Windows.SetWidth(b,c,2);Windows.SetX(b,0,1);Windows.List[b].maximized=1}typeof Windows.List[b].resizeMessage!="undefined"&&sendMsg(Windows.List[b].checknum,Windows.List[b].resizeMessage,eyeParam("arg",c)+eyeParam("arg",d))}},Minimize:function(b,c){Windows.List[b].minimized=c?2:1;Windows.Hide(b);Taskbars.UnfocusEntry(b)},MinimizeButton:function(b,c){if(b=document.getElementById(b+
"_WindowMinimizeButton"))b.style.display=c==2&&b.style.display!="none"||typeof c!="undefined"&&!c?"none":"block"},MinMaxAll:function(){var b;if(Windows.List[Windows.Infos.focus].minimized){for(b in Windows.List)if(Windows.List[b]&&Windows.List[b].minimized==2&&document.getElementById(b).parentNode.id=="eyeApps"){Windows.List[b].minimized=0;Windows.Unhide(b)}Windows.Focus(Windows.Infos.focus)}else for(b in Windows.List)Windows.List[b]&&document.getElementById(b).parentNode.id=="eyeApps"&&Windows.Minimize(b,
1)},MoveAfter:function(b){if(!Windows.List[b].noDrag){Windows.DragAfter(b);if(!Windows.List[b].maximized&&typeof Windows.List[b].moveMessage!="undefined")Windows.List[b].moveChecknum==Windows.List[b].checknum?sendMsg(Windows.List[b].checknum,Windows.List[b].moveMessage,eyeParam("x",xLeft(b))+eyeParam("y",xTop(b))+eyeParam("winName",b)):sendMsg(Windows.List[b].moveChecknum,"saveWinPosition",eyeParam("left",xLeft(b))+eyeParam("top",xTop(b))+eyeParam("winName",b)+eyeParam("appChecknum",Windows.List[b].checknum))}},
MoveBefore:function(b){if(!Windows.List[b].noDrag&&Windows.List[b].maximized!=2){Windows.List[b].x2=xLeft(b);Windows.List[b].y2=xTop(b);Windows.DragBefore(b)}},MoveEvent:function(b,c,d){if(!Windows.List[b].noDrag&&Windows.List[b].maximized!=2&&!Windows.List[b].resizing){Windows.List[b].maximized=0;Windows.List[b].x2+=c;Windows.List[b].y2+=d;Windows.SetX(b,Windows.List[b].x2);Windows.SetY(b,Windows.List[b].y2)}},OnClickTaskbar:function(b){if(Windows.List[b].minimized){Windows.List[b].minimized=0;Windows.Unhide(b);
Windows.Focus(b)}else Windows.Infos.focus==b?Windows.Minimize(b):Windows.Focus(b)},ResizeAfter:function(b,c,d){if(!Windows.List[b].noResize){Windows.DragAfter(b);if(!Windows.List[b].maximized){Windows.List[b].resizing=0;typeof Windows.List[b].resizeMessage!="undefined"&&sendMsg(Windows.List[b].checknum,Windows.List[b].resizeMessage,eyeParam("arg",xWidth(b))+eyeParam("arg",xHeight(b)));if((c==2||d==2)&&typeof Windows.List[b].moveMessage!="undefined")Windows.List[b].moveChecknum==Windows.List[b].checknum?
sendMsg(Windows.List[b].checknum,Windows.List[b].moveMessage,eyeParam("x",xLeft(b))+eyeParam("y",xTop(b))+eyeParam("winName",b)):sendMsg(Windows.List[b].moveChecknum,"saveWinPosition",eyeParam("left",xLeft(b))+eyeParam("top",xTop(b))+eyeParam("winName",b)+eyeParam("appChecknum",Windows.List[b].checknum))}}},ResizeBefore:function(b,c,d){if(!Windows.List[b].noResize&&Windows.List[b].maximized!=2){Windows.List[b].resizing=1;if(c){Windows.List[b].width2=xWidth(b);if(c==2)Windows.List[b].x2=xLeft(b)}if(d){Windows.List[b].height2=
xHeight(b);if(d==2)Windows.List[b].y2=xTop(b)}Windows.DragBefore(b)}},ResizeButton:function(b,c){if(b=document.getElementById(b+"_WindowResizeButton"))b.style.display=c==2&&b.style.display!="none"||typeof c!="undefined"&&!c?"none":"block"},ResizeEvent:function(b,c,d,e,f){if(!Windows.List[b].noResize&&Windows.List[b].maximized!=2){Windows.List[b].maximized=0;if(c==2){Windows.List[b].width2-=d;Windows.SetWidth(b,Windows.List[b].width2);Windows.List[b].x2+=d;Windows.SetX(b,Windows.List[b].x2)}else if(c){Windows.List[b].width2+=
d;Windows.SetWidth(b,Windows.List[b].width2)}if(e==2){Windows.List[b].height2-=f;Windows.SetHeight(b,Windows.List[b].height2);Windows.List[b].y2+=f;Windows.SetY(b,Windows.List[b].y2)}else if(e){Windows.List[b].height2+=f;Windows.SetHeight(b,Windows.List[b].height2)}}},SetHeight:function(b,c,d){var e=document.getElementById(b);if(e){var f=xHeight(e.parentNode);if(typeof d!=="number")d=0;if(d===0&&c>f)c=f;if(d===0&&c<50)c=50;if(d<2&&Windows.List[b].minHeight&&c<Windows.List[b].minHeight)c=Windows.List[b].minHeight;
if(d<2&&Windows.List[b].maxHeight&&c>Windows.List[b].maxHeight)c=Windows.List[b].maxHeight;e.style.height=c+"px"}},SetRawContent:function(b,c,d){if(b=document.getElementById(b+"_Content")){if(d)c=Base64.decode(c);b.innerHTML=c}},SetTitle:function(b,c){var d=document.getElementById(b+"_WindowTitle_text");if(d)d.innerHTML=c;Taskbars.RenameEntry(b,c)},SetWidth:function(b,c,d){var e=document.getElementById(b);if(e){var f=xWidth(e.parentNode);if(typeof d!=="number")d=0;if(d===0&&c>f)c=f;if(d===0&&c<100)c=
100;if(d<2&&Windows.List[b].minWidth&&c<Windows.List[b].minWidth)c=Windows.List[b].minWidth;if(d<2&&Windows.List[b].maxWidth&&c>Windows.List[b].maxWidth)c=Windows.List[b].maxWidth;e.style.width=c+"px"}},SetX:function(b,c,d){b=document.getElementById(b);var e=xWidth(b);if(typeof d=="undefined"||!d){d=b.parentNode.id;var f=xWidth(b.parentNode);if(d!="eyeApps")if(c<0)c=0;else{if(c>f-e)c=f-e}else if(c<25-e)c=25-e;else if(c>f-25)c=f-25}if(b)b.style.left=c+"px"},SetY:function(b,c,d){b=document.getElementById(b);
if(typeof d=="undefined"||!d)if(c<0)c=0;else{d=b.parentNode.id;var e=xHeight(b),f=xHeight(b.parentNode);if(d!="eyeApps"&&c>f-e)c=f-e;else if(c>f-25)c=f-25}if(b)b.style.top=c+"px"},SplitX:function(b){for(var c=xHeight("eyeApps"),d=xWidth("eyeApps")/b.length,e=0,f=0;f<b.length;f++){Windows.SetHeight(b[f],c);Windows.SetWidth(b[f],d);Windows.SetX(b[f],e);Windows.SetY(b[f],0);e+=d}},SplitY:function(b){for(var c=xHeight("eyeApps")/b.length,d=xWidth("eyeApps"),e=0,f=0;f<b.length;f++){Windows.SetHeight(b[f],
...
0}},Unhide:function(b){var c=document.getElementById(b);if(c){if(Windows.List[b].showDragContent)c.style.visibility="visible";else c.style.display="block";if(IEversion&&IEversion<7){c=c.getElementsByTagName("select");for(var d=0;d<c.length;d++)if(c[d].name=="visible")c[d].style.visibility="visible"}typeof Windows.List[b].JS==="object"&&typeof Windows.List[b].JS.Unhide==="function"&&Windows.List[b].JS.Unhide(b)}}};Windows.Infos.focus=0;
