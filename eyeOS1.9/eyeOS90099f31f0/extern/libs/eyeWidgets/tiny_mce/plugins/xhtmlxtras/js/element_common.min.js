...
SXE.insertElement=function(a){var b=SXE.inst.dom.getParent(SXE.focusElement,a.toUpperCase());tinyMCEPopup.execCommand("mceBeginUndoLevel");if(b==null){if(SXE.inst.selection.getContent().length>0){if(tinymce.isIE&&a.indexOf("html:")==0)a=a.substring(5).toLowerCase();insertInlineElement(a);a=tinymce.grep(SXE.inst.dom.select(a));for(var c=0;c<a.length;c++){b=a[c];if(SXE.inst.dom.getAttrib(b,"_mce_new")){b.id="";b.setAttribute("id","");b.removeAttribute("id");b.removeAttribute("_mce_new");setAllCommonAttribs(b)}}}}else setAllCommonAttribs(b);
SXE.inst.nodeChanged();tinyMCEPopup.execCommand("mceEndUndoLevel")};SXE.removeElement=function(a){a=a.toLowerCase();if((elm=SXE.inst.dom.getParent(SXE.focusElement,a.toUpperCase()))&&elm.nodeName.toUpperCase()==a.toUpperCase()){tinyMCEPopup.execCommand("mceBeginUndoLevel");tinyMCE.execCommand("mceRemoveNode",false,elm);SXE.inst.nodeChanged();tinyMCEPopup.execCommand("mceEndUndoLevel")}};SXE.showRemoveButton=function(){document.getElementById("remove").style.display="block"};
SXE.containsClass=function(a,b){return a.className.indexOf(b)>-1?true:false};SXE.removeClass=function(a,b){if(a.className==null||a.className==""||!SXE.containsClass(a,b))return true;for(var c=a.className.split(" "),d="",f=0,g=c.length;f<g;f++)if(c[f]!=b)d+=c[f]+" ";a.className=d.substring(0,d.length-1)};SXE.addClass=function(a,b){if(!SXE.containsClass(a,b))a.className?(a.className+=" "+b):(a.className=b);return true};
function insertInlineElement(a){var b=tinyMCEPopup.editor,c=b.dom;b.getDoc().execCommand("FontName",false,"mceinline");tinymce.each(c.select("span,font"),function(d){if(d.style.fontFamily=="mceinline"||d.face=="mceinline")c.replace(c.create(a,{_mce_new:1}),d,1)})};
