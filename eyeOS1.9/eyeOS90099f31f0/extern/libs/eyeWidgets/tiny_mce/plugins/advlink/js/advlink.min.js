...
function parseLink(a){a=a.replace(new RegExp("&#39;","g"),"'");var b=a.replace(new RegExp("\\s*([A-Za-z0-9.]*)\\s*\\(.*","gi"),"$1"),c=templates[b];if(c){c=c.match(new RegExp("'?\\$\\{[A-Za-z0-9.]*\\}'?","gi"));for(var d="\\s*[A-Za-z0-9.]*\\s*\\(",e="",f=0;f<c.length;f++){d+=c[f].indexOf("'${")!=-1?"'(.*)'":"([0-9]*)";e+="$"+(f+1);c[f]=c[f].replace(new RegExp("[^A-Za-z0-9]","gi"),"");if(f!=c.length-1){d+="\\s*,\\s*";e+="<delim>"}else d+=".*"}d+="\\);?";var g=[];g._function=b;a=a.replace(new RegExp(d,
"gi"),e).split("<delim>");for(f=0;f<c.length;f++)g[c[f]]=a[f];return g}return null}function parseOptions(a){if(a==null||a=="")return[];a=a.toLowerCase();a=a.replace(/;/g,",");a=a.replace(/[^0-9a-z=,]/g,"");a=a.split(",");for(var b=[],c=0;c<a.length;c++){var d=a[c].split("=");if(d.length==2)b[d[0]]=d[1]}return b}
...
"")a.href.value=c}else a.onclick.value=""}function setAttrib(a,b,c){var d=document.forms[0].elements[b.toLowerCase()],e=tinyMCEPopup.editor.dom;if(typeof c=="undefined"||c==null){c="";if(d)c=d.value}if(b=="style")c=e.serializeStyle(e.parseStyle(c));e.setAttrib(a,b,c)}
function getAnchorListHTML(a,b){var c=tinyMCEPopup.editor,d=c.dom.select("a.mceItemAnchor,img.mceItemAnchor"),e="";e+='<select id="'+a+'" name="'+a+'" class="mceAnchorList" o2nfocus="tinyMCE.addSelectAccessibility(event, this, window);" onchange="this.form.'+b+".value=";e+='this.options[this.selectedIndex].value;">';e+='<option value="">---</option>';for(b=0;b<d.length;b++)if((a=c.dom.getAttrib(d[b],"name"))!="")e+='<option value="#'+a+'">'+a+"</option>";e+="</select>";return e}
function insertAction(){var a=tinyMCEPopup.editor,b,c,d;b=a.selection.getNode();checkPrefix(document.forms[0].href);b=a.dom.getParent(b,"A");if(document.forms[0].href.value){tinyMCEPopup.execCommand("mceBeginUndoLevel");if(b==null){a.getDoc().execCommand("unlink",false,null);tinyMCEPopup.execCommand("CreateLink",false,"#mce_temp_url#",{skip_undo:1});c=tinymce.grep(a.dom.select("a"),function(e){return a.dom.getAttrib(e,"href")=="#mce_temp_url#"});for(d=0;d<c.length;d++)setAllAttribs(b=c[d])}else setAllAttribs(b);
if(b.childNodes.length!=1||b.firstChild.nodeName!="IMG"){a.focus();a.selection.select(b);a.selection.collapse(0);tinyMCEPopup.storeSelection()}}else{tinyMCEPopup.execCommand("mceBeginUndoLevel");d=a.selection.getBookmark();a.dom.remove(b,1);a.selection.moveToBookmark(d)}tinyMCEPopup.execCommand("mceEndUndoLevel");tinyMCEPopup.close()}
function setAllAttribs(a){var b=document.forms[0],c=b.href.value,d=getSelectValue(b,"targetlist");setAttrib(a,"href",c);setAttrib(a,"title");setAttrib(a,"target",d=="_self"?"":d);setAttrib(a,"id");setAttrib(a,"style");setAttrib(a,"class",getSelectValue(b,"classlist"));setAttrib(a,"rel");setAttrib(a,"rev");setAttrib(a,"charset");setAttrib(a,"hreflang");setAttrib(a,"dir");setAttrib(a,"lang");setAttrib(a,"tabindex");setAttrib(a,"accesskey");setAttrib(a,"type");setAttrib(a,"onfocus");setAttrib(a,"onblur");
setAttrib(a,"onclick");setAttrib(a,"ondblclick");setAttrib(a,"onmousedown");setAttrib(a,"onmouseup");setAttrib(a,"onmouseover");setAttrib(a,"onmousemove");setAttrib(a,"onmouseout");setAttrib(a,"onkeypress");setAttrib(a,"onkeydown");setAttrib(a,"onkeyup");if(tinyMCE.isMSIE5)a.outerHTML=a.outerHTML}function getSelectValue(a,b){a=a.elements[b];if(!a||a.options==null||a.selectedIndex==-1)return"";return a.options[a.selectedIndex].value}
...
tinyMCEPopup.getLang("advlink_dlg.target_parent")+" (_parent)</option>";d+='<option value="_top">'+tinyMCEPopup.getLang("advlink_dlg.target_top")+" (_top)</option>";for(a=0;a<c.length;a++){var e;if(c[a]!=""){b=c[a].split("=")[0];e=c[a].split("=")[1];d+='<option value="'+b+'">'+e+" ("+b+")</option>"}}d+="</select>";return d}preinit();tinyMCEPopup.onInit.add(init);
