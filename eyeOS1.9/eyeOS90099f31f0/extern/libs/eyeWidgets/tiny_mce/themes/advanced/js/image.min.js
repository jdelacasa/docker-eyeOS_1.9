...
c.execCommand("mceRepaint")}}else{if(c.settings.inline_styles)d.style=this.styleVal;else d=tinymce.extend(d,{vspace:b.vspace.value,hspace:b.hspace.value,border:b.border.value,align:getSelectValue(a,"align")});tinymce.extend(d,{src:a.src.value,alt:a.alt.value,width:a.width.value,height:a.height.value});if((a=c.selection.getNode())&&a.nodeName=="IMG")c.dom.setAttribs(a,d);else{c.execCommand("mceInsertContent",false,'<img id="__mce_tmp" />',{skip_undo:1});c.dom.setAttribs("__mce_tmp",d);c.dom.setAttrib("__mce_tmp",
"id","");c.undoManager.add()}}tinyMCEPopup.close()},updateStyle:function(){var a=tinyMCEPopup.dom,b,c,d=document.forms[0];if(tinyMCEPopup.editor.settings.inline_styles){b=tinyMCEPopup.dom.parseStyle(this.styleVal);if(c=getSelectValue(d,"align"))if(c=="left"||c=="right"){b["float"]=c;delete b["vertical-align"]}else{b["vertical-align"]=c;delete b["float"]}else{delete b["float"];delete b["vertical-align"]}if((c=d.border.value)||c=="0")b.border=c=="0"?"0":c+"px solid black";else delete b.border;if(c=
d.hspace.value){delete b.margin;b["margin-left"]=c+"px";b["margin-right"]=c+"px"}else{delete b["margin-left"];delete b["margin-right"]}if(c=d.vspace.value){delete b.margin;b["margin-top"]=c+"px";b["margin-bottom"]=c+"px"}else{delete b["margin-top"];delete b["margin-bottom"]}b=tinyMCEPopup.dom.parseStyle(a.serializeStyle(b));this.styleVal=a.serializeStyle(b)}},getAttrib:function(a,b){var c=tinyMCEPopup.editor,d=c.dom,f;if(c.settings.inline_styles)switch(b){case "align":if(f=d.getStyle(a,"float"))return f;
if(f=d.getStyle(a,"vertical-align"))return f;break;case "hspace":f=d.getStyle(a,"margin-left");c=d.getStyle(a,"margin-right");if(f&&f==c)return parseInt(f.replace(/[^0-9]/g,""));break;case "vspace":f=d.getStyle(a,"margin-top");c=d.getStyle(a,"margin-bottom");if(f&&f==c)return parseInt(f.replace(/[^0-9]/g,""));break;case "border":f=0;tinymce.each(["top","right","bottom","left"],function(g){g=d.getStyle(a,"border-"+g+"-width");if(!g||g!=f&&f!==0){f=0;return false}if(g)f=g});if(f)return parseInt(f.replace(/[^0-9]/g,
""));break}if(f=d.getAttrib(a,b))return f;return""},resetImageData:function(){var a=document.forms[0];a.width.value=a.height.value=""},updateImageData:function(){var a=document.forms[0],b=ImageDialog;if(a.width.value=="")a.width.value=b.preloadImg.width;if(a.height.value=="")a.height.value=b.preloadImg.height},getImageData:function(){var a=document.forms[0];this.preloadImg=new Image;this.preloadImg.onload=this.updateImageData;this.preloadImg.onerror=this.resetImageData;this.preloadImg.src=tinyMCEPopup.editor.documentBaseURI.toAbsolute(a.src.value)}};
