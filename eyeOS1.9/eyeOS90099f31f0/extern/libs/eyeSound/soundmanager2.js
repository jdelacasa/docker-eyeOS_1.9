...
function SoundManager(smURL, smID) {
  ...
  this.debugMode = false;            // enable debugging output (div#soundmanager-debug, OR console if available+configured) // eyeOS
  this.debugFlash = false;           // enable debugging output inside SWF, troubleshoot Flash/browser issues
  this.useConsole = true;            // use firebug/safari console.log()-type debug console if available
  this.consoleOnly = false;          // if console is being used, do not create/write to #soundmanager-debug
  this.waitForWindowLoad = false;    // force SM2 to wait for window.onload() before trying to call soundManager.onload()
  this.nullURL = 'about:blank';      // path to "null" (empty) MP3 file, used to unload sounds (Flash 8 only) // eyeOS
  ...
  this.defaultOptions = {
    ...
    'volume': 50                   // self-explanatory. 0-100, the latter being the max. // eyeOS
  };
  ...
  _normalizeMovieURL = function(smURL) { // eyeOS
    return 'index.php?version=' + EXTERN_CACHE_VERSION + '&extern=libs/eyeSound/' + _s.movieURL;
  };
...
if (FlashDetect.installed) { // eyeOS if (typeof SM2_DEFER === 'undefined' || !SM2_DEFER) {
